    ...
    try {
        ...
        await imaps.connect(config).then(function (connection) {
            return connection.openBox('INBOX').then(function () {
                var searchCriteria = ['UNSEEN', ['SINCE', dateFormat(date, "mmmm d, yyyy")] ];
                var fetchOptions = {
                    bodies: ['HEADER', 'TEXT', ''],
                };
                    ...
                    messages.forEach(function (item) {
                        ...
                        simpleParser(idHeader+all.body, (err, mail) => {
                            // access to the whole mail object
                             ...
                             date = mail.date 
                              const Ticket = new ticket({from: mail.headers.get('return-path').text,
                            ...
                            email.push({from: mail.headers.get('return-path').text,
                            to: mail.headers.get('delivered-to').text,
                            subject: mail.subject,
                            message : mail.text})
